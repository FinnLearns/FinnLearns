...
* Accept the move to $c'$ with probability $\textbf{min}\left\(1, \frac{\pi (c')}{\pi (c)} \frac{P(c' \rightarrow c)}{P(c \rightarrow c')})$
...
<pre><code class="language-c"> for (double b = betaLower; b <= betaUpper; b+= betaStep)
{
    magnetArray = {};
...
<pre><code class="language-c"> void Ising::updateLattice(double currentBeta, int rand1, double rand2)
{
...
<pre><code class="language-c>"void Ising::findNeighbours()
{
